...
use slint::{Timer, TimerMode, ModelRc, VecModel, Color, SharedString}; // Dodano importy
    ...
    timer.start(TimerMode::Repeated, std::time::Duration::from_millis(16), move || {
        ...
        if let Some(brain) = &app.brain {
            if let Ok((chord, score)) = brain.predict(&spectrum) {
                if score > 1.5 { ui.set_ai_text(format!("AI: {} ({:.1})", chord, score).into()); }
                else { ui.set_ai_text("AI: ...".into()); }
            }
        }
        ...
        // Iterujemy tylko po wybranych w konfiguracji interwałach
        ...
        for (col_idx, &(_random_logic_idx, internal_idx)) in valid_indices.iter().enumerate() {
             ...
             if is_active {
                 ui_colors.push(Color::from_rgb_u8(100, 255, 100)); // Zielony
             } else if is_target {
                 ui_colors.push(Color::from_rgb_u8(255, 255, 0)); // Żółty
             } else if in_delay {
