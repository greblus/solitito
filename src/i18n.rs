...
pub const EN: Strings = Strings {
    ...
    require_onset_hint: "Requires attack evidence for credited notes. In Intervals this covers every detection method and is on by default, with a separate saved choice; a confirmed new chord strum may cover a missed per-note attack. Elsewhere it gates the model only. Requires a model with an onset head.",
...
pub const PL: Strings = Strings {
    ...
    require_onset_hint: "Wymaga dowodu ataku dla zaliczanych dźwięków. W Interwałach obejmuje wszystkie metody wykrywania, jest domyślnie włączone i zapamiętywane osobno; potwierdzone nowe uderzenie akordu może zastąpić przeoczony atak pojedynczej nuty. W pozostałych trybach dotyczy tylko modelu. Wymaga modelu z głowicą ataków.",
