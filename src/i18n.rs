...
pub struct Strings {
    ...
    pub voice_leading: &'static str,
    pub voice_leading_hint: &'static str,
    pub preview: &'static str,
    pub preview_hint: &'static str,
    pub preview_names: &'static str,
    pub preview_tab: &'static str,
    pub preview_neck: &'static str,
    pub preview_note_names: &'static str,
    pub preview_functions: &'static str,
    pub preview_note_names_neck: &'static str,
    pub preview_functions_neck: &'static str,
...
pub const EN: Strings = Strings {
    ...
    single_notes_hint: "Require individual notes. Always on in Scales and Arpeggios. Saved separately for each mode; legato does not require picking every note.",
    require_onset: "Credit only what was struck",
    require_onset_hint: "Require attack evidence, including for CQT in Intervals, Scales, Arpeggios and Formulas. Saved per mode; defaults on only in Intervals, where a confirmed strum can cover a missed tone attack. Turn off for legato. Requires an onset model.",
...
Off: the three notes in any order, shuffled or not.",
    ...
    voice_leading: "Lead the voices",
    voice_leading_hint: "Intervals: each grip is taken where the fingers have least to move from the one before. Off, every chord is taken wherever the neck offers it - the same shape learned all over the neck rather than in one corner of it. The shuffle switches it off as well.",
    preview: "View",
    preview_hint: "The line of intervals says which degrees; the drawings say where the fingers go. On the neck a phrase is drawn in the position it is played, with the note due ringed - which keeps a long study readable. An interval set is a grip and a single note is a place, so those two modes offer the neck or the names and no tablature: tablature is an order of notes, and neither of them is one.",
    preview_names: "Line of intervals",
    preview_tab: "Tablature",
    preview_neck: "On the neck",
    preview_note_names: "Note names",
    preview_functions: "Interval functions",
    preview_note_names_neck: "Note names on the neck",
    preview_functions_neck: "Interval functions on the neck",
    shapes_kind: "Shapes: ",
...
pub const PL: Strings = Strings {
    ...
    single_notes_hint: "Wymaga pojedynczych dźwięków. W Skalach i Arpeggiach zawsze włączone. Zapamiętywane osobno dla każdego trybu; legato nie wymaga szarpania każdej nuty.",
    require_onset: "Zaliczaj tylko to, co uderzone",
    require_onset_hint: "Wymaga dowodu ataku, również dla CQT w Interwałach, Skalach, Arpeggiach i Formułach. Zapis osobny dla trybu; domyślnie włączone tylko w Interwałach, gdzie potwierdzone uderzenie akordu może zastąpić przeoczony atak składnika. Wyłącz dla legato. Wymaga modelu z głowicą ataków.",
...
Wyłączone: trzy dźwięki w dowolnej kolejności, z tasowaniem i bez.",
    ...
    voice_leading: "Prowadź głosy",
    voice_leading_hint: "Interwały: każdy chwyt brany jest tam, gdzie palce mają najmniej do przejścia od poprzedniego. Wyłączone: każdy akord brany jest tam, gdzie gryf go daje — ten sam kształt poznawany po całym gryfie, a nie w jednym jego rogu. Losowanie też to wyłącza.",
    preview: "Widok",
    preview_hint: "Linia interwałów mówi, które stopnie; rysunki mówią, gdzie idą palce. Na podstrunnicy fraza rysowana jest w pozycji, w której się ją gra, z pierścieniem na dźwięku, który jest na kolei — dzięki temu długie studium pozostaje czytelne. Zbiór interwałów jest chwytem, a pojedynczy dźwięk miejscem, więc te dwa tryby dają do wyboru podstrunnicę albo nazwy i nie mają tabulatury: tabulatura jest kolejnością dźwięków, a żadne z nich nią nie jest.",
    preview_names: "Linia interwałów",
    preview_tab: "Tabulatura",
    preview_neck: "Na podstrunnicy",
    preview_note_names: "Tekstowe nazwy dźwięków",
    preview_functions: "Funkcje interwałowe",
    preview_note_names_neck: "Nazwy dźwięków na gryfie",
    preview_functions_neck: "Funkcje interwałowe na gryfie",
