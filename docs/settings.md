...
| **Credit only what was struck** | On by default in Intervals, saved separately: every detection path, including CQT, normally requires a confirmed 0.60 onset rising edge for the credited note after the round starts. A confirmed new chord strum can cover a missed per-note edge when at least two chord-tone attacks and that tone's pitch agree. Older settings also receive this interval default; turning it off is remembered. In other modes the existing shared choice stays off by default and gates only the model. A model without an onset head requires this option off |
