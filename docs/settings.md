...
| **Credit only what was struck** | Saved separately per mode. Defaults on in Intervals and off in Scales, Arpeggios and Formulas. When enabled, CQT also requires a confirmed attack after the round starts in these four modes. Intervals retains its confirmed-strum exception. Disable for legato or a model without an onset head |
