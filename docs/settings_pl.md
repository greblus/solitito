...
| **Zaliczaj tylko to, co uderzone** | W Interwałach domyślnie włączone i zapamiętywane osobno: każda ścieżka wykrywania, także CQT, zwykle wymaga potwierdzonego zbocza ataku 0,60 dla zaliczanej nuty po rozpoczęciu rundy. Potwierdzone nowe uderzenie akordu może zastąpić przeoczone zbocze pojedynczej nuty, jeśli zgadzają się co najmniej dwa ataki składników i wysokość tej nuty. Starsza konfiguracja też otrzymuje tę wartość domyślną; wyłączenie jest zapamiętywane. W pozostałych trybach wspólne ustawienie nadal jest domyślnie wyłączone i obejmuje tylko model. Model bez głowicy ataków wymaga wyłączenia opcji |
