...
also moves after every pass, shuffle or no shuffle: it goes through the three strings it is
worth starting from and turns round each time, up and down alternately. Counted rather than
drawn — three strings and two directions are coprime, so six passes cover all six ways of
taking it, where a fair coin gave five descents in a row and read as a broken exercise.
